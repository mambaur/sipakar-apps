...
class IndicationRepository{
  Future<List<IndicationModel>> getIndication() async{
    try {
      http.Response response = await http.get('http://192.168.43.206/sipakar/api/indication');
  ...
  Future getDataById(String idgejala) async{
    try {
      http.Response response = await http.post('http://192.168.43.206/sipakar/api/indication/getById', body:{'idgejala' : idgejala});
