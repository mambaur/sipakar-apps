...
class DiseaseRepository {
  Future<List<Disease>> getDisease() async{
    try {
      http.Response response = await http.get('http://192.168.43.206/sipakar/api/disease');
  ...
  Future getDataById(String idpenyakit) async{
    try {
      http.Response response = await http.post('http://192.168.43.206/sipakar/api/disease/getById', body:{'idpenyakit' : idpenyakit});
